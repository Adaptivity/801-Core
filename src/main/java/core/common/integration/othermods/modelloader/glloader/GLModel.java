...
public class GLModel {
    ...
    public static final GLMaterial defaultMtl = new GLMaterial();
    ...
    public GLMaterial[] groupMaterials = null;  // holds material for each face group (1 group to 1 material)

    public final boolean useMtlFile;
    
	public GLModel(String filename, boolean useMtlFile) {
		// load OBJ file
		mesh = loadMesh(filename);
		this.useMtlFile = useMtlFile;
	}
    ...
    public GL_Mesh loadMesh(String filename) {
    	if (filename.toLowerCase().endsWith(".obj")) {
    		mesh = new GL_OBJ_Importer().load(filename);
    	} else {
    		mesh = new GL_3DS_Importer().load(filename);
