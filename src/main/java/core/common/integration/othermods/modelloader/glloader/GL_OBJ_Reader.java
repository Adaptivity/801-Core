package core.common.integration.othermods.modelloader.glloader;
...
public class GL_OBJ_Reader {
	...
	public List<float[]> vertices = new ArrayList<float[]>();  // Contains float[3] for each Vertex (XYZ)
	public List<float[]> normals = new ArrayList<float[]>();     // Contains float[3] for each normal
	public List<float[]> textureCoords = new ArrayList<float[]>();  // Contains float[3] for each texture map coord (UVW)
	...
	public void loadobject(BufferedReader br) {
		...
		if (debug) {
			// debug:
			System.out.println("GL_OBJ_Reader: read " + numpolygons() + " faces in " + groups.size() + " groups"); //TODO
			// debug:
			for (int i = 0; i < groups.size(); i++) {
				System.out.println("\tGROUP " + i + " " + groups.get(i).name + " has " + groups.get(i).faces.size() + " faces, material is " + groups.get(i).materialname );
			}
		}
	}
	...
	public void calcDimensions() {
		...
		for (int g = 0; g < groups.size(); g++) {
			ArrayList faces = groups.get(g).faces;
	...
	public int numpolygons() {
		int number = 0;
		for (int i = 0; i < groups.size(); i++) {
			number += groups.get(i).faces.size();
		}
		return number;
	}
	...
	public String getGroupName(int g) {
		return groups.get(g).name;
	}
	public ArrayList getGroupFaces(int g) {
		return groups.get(g).faces;
	}
	public String getGroupMaterialName(int g) {
		return groups.get(g).materialname;
	}
	public int getGroupTriangleCount(int g) {
		return groups.get(g).numTriangles;
	}
	
	public final GL_OBJ_Reader setDebug() {
		debug = true;
		return this;
	}
